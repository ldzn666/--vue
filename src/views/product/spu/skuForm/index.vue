        ...
        <el-form ref="form" label-width="80px">
            ...
            <el-form-item label="图片列表">
                <el-table 
                    ...
                    <el-table-column  prop="img Name" label="名称" width="width"></el-table-column>
...
export default {
    ...
    methods: {
        async getData(category1Id,category2Id,spu){
            ...
            this.spu = spu;
            let resImage = await this.$API.spu.reqSpuImgList(spu.id);
            if(resImage.code == 200){
                let list = resImage.data;
                list.forEach((item) => {
                    item.isDefault = 0;
                })
                this.spuImageList = list;
            }
            let resSale = await this.$API.spu.reqSpuSaleAttrList(spu.id);
            if(resSale.code == 200){
                this.spuSaleAttrList = resSale.data;
            }
            let resInfo = await this.$API.spu.reqAttrInfoList(category1Id,category2Id,spu.category3Id);
            if(resInfo.code == 200){
                this.spuAttrInfoList = resInfo.data;
            }
        },
        ...
        async save(){
            //整理参数
            const {spuAttrInfoList,spuSaleAttrList,skuInfo} = this;
            // 方法1: 
            // let arr = [];
            ...
            // })
            //方法2:
            skuInfo.spuAttrInfoList = spuAttrInfoList.reduce((prev,item)=>{
                if(item.attrIdAndValueId){
                    const [attrId,valueId] = item.attrIdAndValueId.split(':');
